cd /Users/utchal/Documents/Stock-MF_Demo/stock-mfund-demo/backend

# backup current file (timestamped)
cp main.py main.py.bak.$(date +%s)

# overwrite main.py with the corrected production-ready version
cat > main.py <<'PY'
#!/usr/bin/env python3
"""
...
Features:
- Clean, structured endpoints: /analysis (POST), /news (GET), /analysis/export (GET)
- Defensive error handling and JSON responses
- Structured logging
- Uses yfinance and feedparser with timeouts / fallbacks
- Safe handling of missing data to avoid returning `null`
- Simple in-memory caching for news per symbol (short TTL) to reduce RSS hits during dev
"""
...
# ---------- Configuration ----------
NEWS_CACHE_TTL = 60 * 5  # seconds (5 minutes) caching for RSS in dev
MAX_NEWS_ITEMS = 8
...
# ---------- App ----------
app = FastAPI(
    title="Stock & Mutual Fund Analysis - Demo (Patched, Prod-ready)",
    version="1.0"
)
...
def safe_history_fetch(ticker: yf.Ticker, start_date: date, end_date: date, interval="1d"):
    """
    Wrapper around yfinance history fetch. Returns pandas DataFrame or None on failure.
    """
    try:
...
def get_price_on(ticker: yf.Ticker, target_date: date):
    """
    Safely get the closing price for the ticker on or near target_date.
    Returns tuple (price: float | None, timestamp_str: str | "")
    """
    try:
...
# ---------- News helper ----------
def _fetch_news_rss(symbol: str) -> List[Dict[str, str]]:
    """
    Fetches Google News RSS for the symbol. Returns list of dict (title, link, pubDate).
    This function is defensive and returns [] on any error.
    """
    try:
        ...
        for entry in feed.entries[:MAX_NEWS_ITEMS]:
            items.append({
                ...
                "pubDate": getattr(entry, "published", "") or ""
            })
        return items
    except Exception:
        logger.exception("Failed to fetch news RSS for %s", symbol)
        return []


def get_news(symbol: str) -> List[Dict[str, str]]:
    """
    Cached wrapper around RSS fetch to reduce repeated requests while developing.
    """
    now_ts = time.time()
    cache = _news_cache.get(symbol)
    if cache and (now_ts - cache.get("ts", 0) < NEWS_CACHE_TTL):
        return cache.get("items", [])

    items = _fetch_news_rss(symbol)
    _news_cache[symbol] = {"ts": now_ts, "items": items}
    return items
...
def analyze_ticker(symbol: str, start_str: str) -> Dict[str, Any]:
    """
    Returns a dict of metrics for a ticker symbol from start_str (DD-MM-YYYY).
    """
    try:
    ...
    # current price: use a few days back to avoid today's incomplete data
    today = _now_date()
...
def news(symbol: str):
    """
    GET /news?symbol=RELIANCE.NS
    """
    try:
        items = get_news(symbol)
        return JSONResponse({"symbol": symbol, "headlines": items})
    except Exception as exc:
...
def analysis(payload: AnalysisRequest):
    """
    POST /analysis
    Body: { "symbol": "...", "start_date": "DD-MM-YYYY", "investment_amount": 100000 }
    """
    try:
        ...
        # Return JSON error with 500 so client doesn't get `null`
